...
public class Solo_Game_screen extends ScreenAdapter {
    ...
    public Solo_Game_screen (SOSGame game ) {
        ...
        humanPlayer1.setPosition((THEBOARD.getX()+(THEBOARD.getWidth()/100)*10),(THEBOARD.getY()+THEBOARD.getHeight())+(THEBOARD.getHeight()/5));
        humanPlayer1.setSize((SOSGame.WIDTH/100)*13  , (SOSGame.HEIGHT/100)*11);

        android = new Sprite(atlas.createSprite("android_new"));
        android.setPosition((THEBOARD.getX()+(THEBOARD.getWidth()/100)*77),(THEBOARD.getY()+THEBOARD.getHeight())+(THEBOARD.getHeight()/5));
        android.setSize((SOSGame.WIDTH/100)*13 , (SOSGame.HEIGHT/100)*11);
        ...
        float x=(0+backButton.getWidth()/2);
        for(int a=0;a<row; a++ )
        {
    ...
    public void playSolo( ) {
                                     ...
                                     int button)
            {
                ...
                for (int i = 0; i < row; i++) {
                    for (int g = 0; g < column; g++) {
                        if(currentPlayer==p1 && CELLS[i][g].isPressed() == true&& (choosenValue==Cell.CellValue.S_cell ||choosenValue==Cell.CellValue.O_cell) && board.cellAtPosition(new CellPosition(i,g)).getValue() == Cell.CellValue.EMPTY)
                        {
                        ...
                        if((currentPlayer == p2) && (board.emptyCellPositions(board).size()!=0))
                            {
                                ...
                                countofMoves++;
                                currentPlayer=p1;
                                CELLS[CP.getRow()][CP.getColumn()].addAction(repeatAction);
                                lastplay=CP;
                            }
                        ...
                        if(board.emptyCellPositions(board).size() == 0)
                        {
                            ...
                            {
                                winner=p1;
                                winn="The Winner is: "+winner.getName();
                                crown.setSize((SOSGame.WIDTH/100)*15 , (SOSGame.HEIGHT/100)*5);
                                crown.setPosition(humanPlayer1.getX()+((humanPlayer1.getWidth()-crown.getWidth())/2),(humanPlayer1.getY()+(humanPlayer1.getHeight()))); //(SOSGame.WIDTH/100)*18,(SOSGame.HEIGHT/100)*92
                            ...
                            }else if(score2>score1)
                            {
                                ...
                                winn="The Winner is: "+winner.getName();
                                crown.setSize((SOSGame.WIDTH/100)*15 , (SOSGame.HEIGHT/100)*5);
                                crown.setPosition(android.getX()+((android.getWidth()-crown.getWidth())/2),(android.getY()+(android.getHeight())));
                                actorGroup.addActor(crown);
                            ...
                            end.setPosition(THEBOARD.getX()+(THEBOARD.getWidth()-(end.getWidth()))/2,(SOSGame.HEIGHT/100)*15);
                            actorGroup.addActor(end);
                            backButton.setPosition((THEBOARD.getWidth()/100)*38,(THEBOARD.getHeight()/100)*2);
                            renewButton.setPosition((THEBOARD.getWidth()/100)*65,(THEBOARD.getHeight()/100)*2);
                        }
                        }
                    }
        ...
        Label.LabelStyle l1=new Label.LabelStyle(other_font,Color.valueOf(("6BD6D7"))); //playerName1
        Label.LabelStyle l3=new Label.LabelStyle(other_font,Color.valueOf(("eb4e6b"))); //player2 namered
        Label.LabelStyle l2=new Label.LabelStyle(other_font,Color.valueOf(("FFFF99"))); //player2 namered

        p1name=new Label(playerName1,l1);
        p1name.setPosition((humanPlayer1.getX()+((humanPlayer1.getWidth()-p1name.getWidth())/2)),humanPlayer1.getY()-(p1name.getHeight()));

        p2name=new Label(playerName2,l3);
        p2name.setPosition((android.getX()+((android.getWidth()-p2name.getWidth())/2)),android.getY()-(p2name.getHeight()));


        if(Options.S_colors==Options.S_colors.Yellows)
        {p2name.setStyle(l2);}

        solo_stage.addActor(p1name);
        solo_stage.addActor(p2name);
    ...
    public void render ( float delta) {
        ...
        SOSGame.batch.draw(humanPlayer1,(THEBOARD.getX()+(THEBOARD.getWidth()/100)*10),(THEBOARD.getY()+THEBOARD.getHeight())+(THEBOARD.getHeight()/5),(SOSGame.WIDTH/100)*13, (SOSGame.HEIGHT/100)*11);
        SOSGame.batch.draw(vs,(THEBOARD.getX()+(THEBOARD.getWidth()/100)*45),(THEBOARD.getY()+THEBOARD.getHeight())+(THEBOARD.getHeight()/8),(SOSGame.WIDTH/100)*12, (SOSGame.HEIGHT/100)*13);
        SOSGame.batch.draw(android,(THEBOARD.getX()+(THEBOARD.getWidth()/100)*77),(THEBOARD.getY()+THEBOARD.getHeight())+(THEBOARD.getHeight()/5),(SOSGame.WIDTH/100)*13, (SOSGame.HEIGHT/100)*11);
        other_font.setColor(Color.WHITE);

        other_font.draw(SOSGame.batch,ScoreName2,(android.getX()+((android.getWidth()-(ScoreName2.length()*4))/2)),p2name.getY()-(p2name.getHeight()/5));
        other_font.draw(SOSGame.batch,ScoreName1,(humanPlayer1.getX()+((humanPlayer1.getWidth()-(ScoreName1.length()*4))/2)),p1name.getY()-(p2name.getHeight()/5));
        SOSGame.batch.end();
